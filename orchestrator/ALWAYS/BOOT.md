...
每次 Program 完成后，自动收集以下反馈：

| 反馈项 | 收集时机 | 用途 |
|--------|----------|------|
| 流程顺畅度 | Review 阶段 | 识别阻塞步骤 |
| 文档清晰度 | 每阶段结束 | 优化步骤描述 |
| 确认点合理性 | Decision 阶段 | 调整确认时机 |
| 产物质量 | Execute 阶段 | 改进生成模板 |

反馈存储：`workspace/feedback.md`（可选，不强制填写）
