...
- Phase 8 功能归档已完成
- 当前 Program 涉及门面服务（`mall-admin`、`mall-tob-service`、`mall-toc-service`、`mall-ai`）Controller 层接口的新增或变更
...
- 本 Program 涉及的门面服务 Controller 类（`XxxAdminController` / `XxxAppController` / `XxxMerchantController`）
- 对应的请求/响应 VO、DTO 类
...
示例：
- `JobTypeAdminController`（`@Tag` 描述：岗位类型管理）→ `岗位类型管理-JobTypeAdminApi.md`
- `StyleConfigAdminController`（`@Tag` 描述：人设风格管理）→ `人设风格管理-StyleConfigAdminApi.md`
- `EmployeeAppController`（`@Tag` 描述：智能员工信息）→ `智能员工信息-EmployeeAppApi.md`
...
Phase 9 执行计划
- 当前阶段: Swagger 文档生成
- 执行方式: Skill/swagger-doc-generation（主 Agent 直接执行）
...
- 提取返回值类型（`CommonResult<XxxVO>` → 读取 `XxxVO` 类全部字段）
- 从字段注解中提取校验约束：
...
module: {ControllerName 去掉 Controller 后缀的 PascalCase，如 JobTypeAdmin}
title: {中文功能描述}
version: v{N}.{M}.0
...
- **完整路径表示 (Full Path Representation)**：API 路径必须包含 `/{service-name}` 前缀，如 `/mall-admin/admin/api/v1/job-types`
...
- **DTO 字段对齐 (DTO Field Alignment)**：分页接口响应 data 内容必须严格按以下结构描述（字段命名不得替换）：
  ...
  | `items` | Array\<{XxxVO}\> | 分页数据列表，对应 `CommonResult.PageData.items` |
