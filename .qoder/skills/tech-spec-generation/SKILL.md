...
| **规格生成约束** | `.qoder/rules/code-generation/12-spec-generation-constraints.md` | **规范引用索引 + 自动化应用描述（各约束详见对应权威规范文件）** |
...
基于 `12-spec-generation-constraints.md` 规范引用索引及对应权威规范文件，自动应用约束到规格定义：
...
**规范合规性检查（通过 12-spec-generation-constraints.md 索引指向各权威规范文件）**：
