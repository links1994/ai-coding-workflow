...
implementation:
  ...
  files:
    ...
    - path: mall-agent-employee-service/controller/inner/XxxInnerController.java
      type: controller
      service: mall-agent-employee-service
      layer: application
      
    - path: mall-admin/controller/admin/XxxAdminController.java
      type: controller
      service: mall-admin
      layer: facade
```
...
**4.4 数据模型约束应用**
- 自动根据表名生成 DO 类名（aim_xxx → AimXxxDO）
...
- [ ] DTO 命名符合规范（XxxRequest/XxxResponse/XxxApiRequest/XxxApiResponse）
